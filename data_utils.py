...
import spatial_similarity as spatial_com            # Importing the required modules from the project
import temporal_similarity as temporal_com
import pickle                                       # Importing pickle, yaml, and os libraries
import yaml
import os
...
# Defining a function to load the road network from the specified file
def load_netowrk(dataset):
    ...
    # Reading the node features from the specified file
    node_embeddings = np.load(node_embedding_path)

    # Reading the edge information from the specified file
    ...
    # Constructing the data object for the graph
